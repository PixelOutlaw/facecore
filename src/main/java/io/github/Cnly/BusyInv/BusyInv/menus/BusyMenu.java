...
public class BusyMenu implements IBusyMenu {
     ...
     *
     * @param inventoryType One of {@link org.bukkit.event.inventory.InventoryType}
    ...
    /**
     * Runs a task to close the inventory safely.
     * This method should be overridden with:
     * {@code this.closeInventorySafely(YourPluginInstance, p)}
     * when BusyInv is not a standalone plugin!
     *
     * @param p The player
     */
    protected void closeInventorySafely(final Player p) {
        this.closeInventorySafely(FacecorePlugin.getInstance(), p);
    }
